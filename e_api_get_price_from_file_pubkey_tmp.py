...
# コマンド用パラメーター -------------------    
#
FNAME_INPUT = 'price_list_in.csv'   # 取得する情報コードと銘柄を読み込むファイル名。カレントディレクトリに存在すること。
FNAME_OUTPUT = 'price_list_out.csv'   # 書き込むファイル名。カレントディレクトリに上書きモードでファイルが作成される。
...
if __name__ == "__main__":
    ...
    # my_p_no = my_p_no + 1
    # my_login_property.p_no = my_p_no
    # # 更新した"p_no"を保存する。
    ...
    # 口座情報をファイルから読み込む。
    func_get_acconut_info(FNAME_URL_INFO, my_account_property)
    ...
    # 現在（前回利用した）のp_noをファイルから取得する
    my_p_no = func_get_p_no(FNAME_INFO_P_NO)
    my_login_property.p_no = my_p_no + 1
    # "p_no"を保存する。
    ...
    func_read_price_list(FNAME_INPUT, my_code, my_column)

    # 株価情報の出力ファイルに１行目タイトルを書き込む。
    func_write_price_title(FNAME_OUTPUT, my_column)
            ...
            if dic_return is not None:
                ...
                if dic_return.get('p_errno') != '-2' and dic_return.get('p_errno') != '2':
                    ...
                    if list_return is not None:
                        # 取得した株価情報を追記モードでファイルに書き込む。
                        func_write_price_list(FNAME_OUTPUT, list_return, my_column)
                else:
                    print()
